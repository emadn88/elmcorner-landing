...
<head>
    ...
    <title>{{ config('app.name', 'ElmCorner') }} - Online Islamic Academy</title>
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 bg-gray-900/95 backdrop-blur-sm shadow-md border-b border-gray-800" x-data="{ mobileMenuOpen: false }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-20">
                ...
                <div class="flex items-center">
                    <a href="{{ route('home', ['locale' => app()->getLocale()]) }}" class="flex items-center space-x-3">
                        <img src="{{ asset('images/logo.png') }}" alt="ElmCorner Logo" class="h-12 w-auto">
                ...
                <div class="hidden md:flex items-center space-x-6 rtl:space-x-reverse rtl:space-x-6">
                    <a href="#features" class="text-gray-300 hover:text-islamic-green-400 transition-colors px-2">{{ __('messages.features_title') }}</a>
                    <a href="#about" class="text-gray-300 hover:text-islamic-green-400 transition-colors px-2">{{ __('messages.about_title') }}</a>
                    <a href="#courses" class="text-gray-300 hover:text-islamic-green-400 transition-colors px-2">{{ __('messages.courses_title') }}</a>
                    <a href="#testimonials" class="text-gray-300 hover:text-islamic-green-400 transition-colors px-2">{{ __('messages.testimonials_title') }}</a>
                    <a href="https://wa.me/1234567890?text=Hello%2C%20I%20would%20like%20to%20enroll%20in%20ElmCorner" target="_blank" rel="noopener noreferrer" class="btn-primary ml-2 rtl:ml-0 rtl:mr-2">{{ __('messages.cta_enroll') }}</a>
                    
                    <!-- Language Switcher -->
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('home', ['locale' => 'en']) }}" 
                           class="px-3 py-1 rounded {{ app()->getLocale() === 'en' ? 'bg-islamic-green-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                            EN
                        </a>
                        <a href="{{ route('home', ['locale' => 'ar']) }}" 
                           class="px-3 py-1 rounded {{ app()->getLocale() === 'ar' ? 'bg-islamic-green-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                            AR
                        </a>
                    </div>
                </div>
            ...
            <div x-show="mobileMenuOpen" 
                ...
                <a href="#features" class="block py-2 text-gray-300 hover:text-islamic-green-400">{{ __('messages.features_title') }}</a>
                <a href="#about" class="block py-2 text-gray-300 hover:text-islamic-green-400">{{ __('messages.about_title') }}</a>
                <a href="#courses" class="block py-2 text-gray-300 hover:text-islamic-green-400">{{ __('messages.courses_title') }}</a>
                <a href="#testimonials" class="block py-2 text-gray-300 hover:text-islamic-green-400">{{ __('messages.testimonials_title') }}</a>
                <a href="https://wa.me/1234567890?text=Hello%2C%20I%20would%20like%20to%20enroll%20in%20ElmCorner" target="_blank" rel="noopener noreferrer" class="block py-2 btn-primary text-center">{{ __('messages.cta_enroll') }}</a>
                <div class="flex items-center space-x-2 mt-4">
                    <a href="{{ route('home', ['locale' => 'en']) }}" 
                       class="flex-1 text-center px-3 py-1 rounded {{ app()->getLocale() === 'en' ? 'bg-islamic-green-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700' }}">
                        EN
                    </a>
                    <a href="{{ route('home', ['locale' => 'ar']) }}" 
                       class="flex-1 text-center px-3 py-1 rounded {{ app()->getLocale() === 'ar' ? 'bg-islamic-green-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700' }}">
                        AR
                    </a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
                ...
                <div>
                    <div class="flex items-center space-x-3 mb-6">
                        <img src="{{ asset('images/logo.png') }}" alt="ElmCorner Logo" class="h-12 w-auto drop-shadow-lg">
