                ...
                <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 md:pb-16">
                    ...
                    <div class="mb-4 fade-in-on-scroll">
                        <span class="inline-flex items-center gap-2 px-4 py-2 bg-islamic-green-500/20 backdrop-blur-sm rounded-full border border-islamic-green-400/30 text-islamic-green-300 text-sm font-semibold">
                            ...
                            {{ app()->getLocale() === 'ar' ? 'مقال إسلامي' : 'Islamic Article' }}
                        </span>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-6 fade-in-on-scroll">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-xs text-gray-400 uppercase tracking-wider">{{ app()->getLocale() === 'ar' ? 'الكاتب' : 'Author' }}</div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-xs text-gray-400 uppercase tracking-wider">{{ app()->getLocale() === 'ar' ? 'التاريخ' : 'Published' }}</div>
                                <time datetime="{{ $post->published_at ? $post->published_at->toIso8601String() : $post->created_at->toIso8601String() }}" class="text-white font-semibold">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-xs text-gray-400 uppercase tracking-wider">{{ app()->getLocale() === 'ar' ? 'المشاهدات' : 'Views' }}</div>
                                <div class="text-white font-semibold">{{ number_format($post->views_count) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <nav class="py-8 fade-in-on-scroll" aria-label="Breadcrumb">
                    <ol class="flex items-center gap-2 text-sm">
                        <li>
                            <a href="{{ route('home', ['locale' => app()->getLocale()]) }}" class="text-gray-400 hover:text-islamic-green-400 transition-colors">
                                {{ app()->getLocale() === 'ar' ? 'الرئيسية' : 'Home' }}
                            </a>
                        </li>
                        <li class="text-gray-600">/</li>
                        <li>
                            <a href="{{ route('blog.index', ['locale' => app()->getLocale()]) }}" class="text-gray-400 hover:text-islamic-green-400 transition-colors">
                                {{ app()->getLocale() === 'ar' ? 'المدونة' : 'Blog' }}
                            </a>
                        </li>
                        <li class="text-gray-600">/</li>
                        <li class="text-gray-300 truncate max-w-xs">{{ \Illuminate\Support\Str::limit($post->title, 40) }}</li>
                    </ol>
                </nav>
                    ...
                    <div class="bg-gradient-to-br from-gray-800/80 to-gray-700/80 backdrop-blur-sm rounded-2xl p-8 border border-gray-600/50 shadow-xl">
                        <div class="flex items-center justify-between mb-6">
                            <div>
                                <h3 class="text-2xl font-bold text-white mb-2">
                                    {{ app()->getLocale() === 'ar' ? 'شارك هذه المقالة' : 'Share this article' }}
                                </h3>
                                <p class="text-gray-400 text-sm">
                                    {{ app()->getLocale() === 'ar' ? 'ساعد الآخرين على اكتشاف هذا المحتوى' : 'Help others discover this content' }}
                                </p>
                            </div>
                        ...
                        <div class="flex flex-wrap gap-4">
                            ...
                            <button onclick="navigator.clipboard.writeText('{{ url()->current() }}')" 
                                ...
                                <span>{{ app()->getLocale() === 'ar' ? 'نسخ الرابط' : 'Copy Link' }}</span>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-12 fade-in-on-scroll">
                    <div class="bg-gradient-to-br from-gray-800/80 to-gray-700/80 backdrop-blur-sm rounded-2xl p-8 border border-gray-600/50 shadow-xl">
                        <div class="flex items-start gap-6">
                            ...
                            <div class="flex-1">
                                <h4 class="text-xl font-bold text-white mb-2">{{ $post->author_name }}</h4>
                                <p class="text-gray-400 mb-4">
                                    {{ app()->getLocale() === 'ar' ? 'كاتب ومؤلف في التعليم الإسلامي' : 'Writer and author in Islamic education' }}
                                </p>
                                <div class="flex items-center gap-4 text-sm text-gray-500">
                                    <span>{{ app()->getLocale() === 'ar' ? 'مقالات منشورة:' : 'Published articles:' }} 5+</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p class="text-gray-400">
                                {{ app()->getLocale() === 'ar' ? 'اكتشف المزيد من المحتوى المشابه' : 'Discover more similar content' }}
                            </p>
                        </div>
                        ...
                        <article class="group bg-gradient-to-br from-gray-800/90 to-gray-700/90 backdrop-blur-sm rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-600/50">
                            ...
                            <div class="p-6">
                                <div class="flex items-center gap-2 mb-3 text-xs text-gray-500">
                                    <time>{{ $relatedPost->formatted_published_date }}</time>
                                    <span>•</span>
                                    <span>{{ $relatedPost->views_count }} {{ app()->getLocale() === 'ar' ? 'مشاهدة' : 'views' }}</span>
                                </div>
                                ...
                                <a href="{{ route('blog.show', ['locale' => app()->getLocale(), 'slug' => app()->getLocale() === 'ar' ? $relatedPost->slug_ar : $relatedPost->slug_en]) }}" 
                ...
                <div class="text-center pb-16 fade-in-on-scroll">
                    <a href="{{ route('blog.index', ['locale' => app()->getLocale()]) }}" 
                       class="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-islamic-green-600 to-islamic-teal-600 hover:from-islamic-green-700 hover:to-islamic-teal-700 text-white font-semibold rounded-xl transition-all duration-300 transform hover:-translate-y-1 hover:shadow-xl">
                        ...
                        <span>{{ app()->getLocale() === 'ar' ? 'العودة إلى المدونة' : 'Back to Blog' }}</span>
                    </a>
                </div>
            </div>
        </div>
